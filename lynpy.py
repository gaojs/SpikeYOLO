...
try:
    ...
    print("------*******------")
    print("Please install pylynchipsdk with whl and try again")
    print("  pylynchipsdk whl in /usr/local/lynxi/sdk/pysdk/whl/ dir")
    print("  use pip3 install /usr/local/lynxi/sdk/pysdk/whl/pylynchipsdk-1.x.0-py3x-none-xx.whl")
    print("------*******------")
    sys.exit(1)
...
class Tensor(object):
    ...
    def __del__(self):
        ...
        try:
            if self.devptr != None and (self.__child == False):
                sdk.lyn_free(self.devptr)
            if self.context != None and (self.__child == False):
                sdk.lyn_set_current_context(self.context)
                sdk.lyn_destroy_context(self.context)
        except Exception as e:
            # print("[ERROR] free Tensor resource")
            pass 
    ...
    def cpu(self):
        ...
        if self.__numpydata is None:
            self.__numpydata = np.empty(self.data_size, dtype=np.byte)   # 输出总长度字节数
            self.__update_numpydata_info()
...
class Model(object):
    ...
    def __init__(self, dev_id=0, path=None, stream=None, sync=True):
        ...
        # self.input_list()    # print input info
    ...
    def __del__(self):
        ...
        # sdk.lyn_destroy_context(self.context)
        try:
            if self.model != None: sdk.lyn_unload_model(self.model)
            if self.stream != None: sdk.lyn_destroy_stream(self.stream)
            if self.context != None: sdk.lyn_destroy_context(self.context)
        except Exception as e:
            # print("[ERROR] free Model resource")
            pass 
    ...
    def output_list(self):
        ...
        if self.__output_list is None:
            ...
            for i in range(tensor_num):
                ...
                # name, ret1 = sdk.lyn_model_get_output_tensor_name_by_index(self.model, i); print("[lynpy] output",i,name)
    ...
    def input_list(self):
        ...
        if self.__input_list is None:
            ...
            for i in range(tensor_num):
                ...
                # name, ret1 = sdk.lyn_model_get_input_tensor_name_by_index(self.model, i); print("[lynpy] input",i,name)
