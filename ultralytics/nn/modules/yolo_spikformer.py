...
def expand_tensor_cumulative(tensor, max_value=4):
    ...
    # 降维同等替换 ---------------------------------------------------------------
    # 展平为 2D -> shape: [T, B*C*H*W]
    tensor_2d = tensor.view(T, -1)

    less_results = []
    for step in range(max_value):
        # 标量与张量直接比较，避免大张量广播
        binary_step = (step < tensor_2d).float()
        less_results.append(binary_step) 

    binary = torch.stack(less_results, dim=1)
    # 直接 Reshape 恢复原始维度结构 -> shape: [T * max_value, B, C, H, W]
    binary = binary.reshape(T * max_value, B, C, H, W)    
...
class MultiSpike4(nn.Module):  # 直接调用实例化的quant6无法实现深拷贝。解决方案是像下面这样用嵌套的类
    ...
    def forward(self, x):
        ...
        return self.quant4.forward(x)              # lynxi
